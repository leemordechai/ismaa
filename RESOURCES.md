...
- **Manifest generator:** `node scripts/gen-manifest.mjs [unit.json] [out.json] [--only-missing]` walks a unit's content JSON and emits every TTS job it references (word, example, dialogue full + per-line, monologue, grammar example, cloze completed-sentence). Authoring a new unit = write JSON with audio paths, run the generator, run tts.mjs. Slow-pace styles are baked into the generator (learner feedback 2026-06: "a bit slower") — words very slow, sentences/dialogues noticeably slower than natural with clear articulation, dialect hints retained. The in-app 1×/0.8× header toggle slows playback further client-side.
- **Path conventions:** word `audio/tts/u1/<id>.wav`, example `<id>-ex.wav`, dialogue `d<n>.wav` + lines `d<n>-lNN.wav`, monologue `listen<n>.wav`, grammar `g<lesson>-<point>-<ex>.wav`, cloze `c<lesson>-<i>.wav`.
...
- **Quota reality (learned 2026-06-10):** the key's free tier caps each TTS model at **100 requests/model/day** (`generate_requests_per_model_per_day`; resets midnight Pacific). A fully-voiced unit ≈ 160 files, so either split a unit's generation across `gemini-2.5-flash-tts` + `gemini-3.1-flash-tts-preview` (separate quotas — use `gen-manifest.mjs --model=...`), across two days, or enable billing. Failed jobs with `finishReason: OTHER` still consume quota; 429s don't.
