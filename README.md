...
**Arc 1 complete (Units 1–4) + Arc 2 underway (Units 5–6)** — 36 lessons, 300 vocab items, real street/news clips per unit, ~950 audio files at slow learner pace. Unit 5 خبر عاجل (the news register: anchor-فصحى vs street-عامية, with MSA-styled anchor TTS) is live; Unit 6 الضفة is authored and compiled, its audio waiting on the next TTS quota day (manifests `scripts/u6-audio-*.json` ready to run, then flip its trajectory status). Exercise engine: quiz + tap-to-match + cloze after every listening section, randomized answers, 1×/0.8× speed toggle, SRS review with audio. Unit production is compiler-driven — see [AUTHORING.md](AUTHORING.md); `content-src/TAUGHT.md` is the generated inventory of everything taught (regenerate via `scripts/inventory.mjs`). Next: Unit 6 audio, word-bank onboarding triage, then Unit 7 القدس.
